...
`;var me=U(`group-select`),D=U(`message-mode`),O=U(`message-input`),he=U(`private-key`),ge=U(`public-key`),k=U(`encrypt-output`),A=U(`decrypt-output`),j=U(`rerand-output`),M=U(`homo-output`),N=U(`mixnet-output`),P=U(`crack-output`),F=U(`kreuse-output`),I=U(`maul-output`),L=U(`auth-output`),_e=U(`sign-output`),ve=U(`sigreuse-output`),R=v,z=null,B=null,V=null,ye=null,H=null;function U(e){let t=document.getElementById(e);if(!t)throw Error(`Missing element #${e}`);return t}var be={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`};function W(e){return e.replace(/[&<>"']/g,e=>be[e])}function G(e,t=``){return`<span class="tok${t?` `+t:``}">${W(String(e))}</span>`}function K(e){return`<span class="tok ${e?`ok`:`bad`}">${e?`Yes`:`No`}</span>`}function q(e,t){e.dataset.state=`ok`,e.innerHTML=t}function J(e,t){e.dataset.state=`error`,e.innerHTML=`<span class="tok bad">⚠ Error:</span> ${W(t instanceof Error?t.message:`Unknown failure.`)}`}function Y(e,t=96){let n=e.toString();return n.length<=t?n:`${n.slice(0,40)}...${n.slice(-40)} (digits=${n.length})`}function xe(){return me.value===`rfc3526`?b:v}function Se(){z&&(he.textContent=R.isToy?z.privateKey.toString():`${Y(z.privateKey,40)} [hidden by default in production]`,ge.textContent=Y(z.publicKey))}function Ce(){if(D.value===`text`)return p(O.value,R);let e=O.value.trim();if(!/^\d+$/.test(e))throw Error(`Integer mode requires a positive decimal integer.`);let t=BigInt(e);if(t<=0n||t>=R.p)throw Error(`Message must satisfy 1 <= m <= p-1 (p=${R.p}).`);return t}function X(e){return`(${G(Y(e.c1),`c1`)}, ${G(Y(e.c2),`c2`)})`}function Z(){if(!z)throw Error(`Generate a keypair first.`);return z}document.querySelectorAll(`.copy-btn`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.dataset.copy;if(!t)return;let n=document.getElementById(t)?.textContent?.trim();if(n)try{await navigator.clipboard.writeText(n);let t=e.textContent;e.textContent=`Copied`,e.classList.add(`copied`),window.setTimeout(()=>{e.textContent=t,e.classList.remove(`copied`)},1400)}catch{e.textContent=`Copy failed`,window.setTimeout(()=>{e.textContent=`Copy`},1400)}})}),(()=>{let t=document.getElementById(`viz-container`),n=document.getElementById(`viz-exp`),r=document.getElementById(`viz-exp-out`),i=document.getElementById(`viz-readout`);if(!t||!n||!r||!i)return;let{g:o,p:s,q:c}=v,l=Number(c-1n),u=Number(s-1n),d=e=>46+e/l*700,f=e=>346-e/u*332,p=``,m=1n;for(let e=0;e<=l;e+=1)p+=`<circle cx="${d(e).toFixed(1)}" cy="${f(Number(m)).toFixed(1)}" r="1.4" class="viz-pt"/>`,m=m*o%s;t.innerHTML=`
    ...
    </svg>`;let h=t.querySelector(`#viz-marker`),g=t.querySelector(`#viz-vline`),_=t.querySelector(`#viz-hline`);function y(t){let n=e(o,BigInt(t),s),a=d(t),c=f(Number(n));h?.setAttribute(`cx`,a.toFixed(1)),h?.setAttribute(`cy`,c.toFixed(1)),g?.setAttribute(`x1`,a.toFixed(1)),g?.setAttribute(`y1`,c.toFixed(1)),g?.setAttribute(`x2`,a.toFixed(1)),g?.setAttribute(`y2`,`346`),_?.setAttribute(`x1`,`46`),_?.setAttribute(`y1`,c.toFixed(1)),_?.setAttribute(`x2`,a.toFixed(1)),_?.setAttribute(`y2`,c.toFixed(1)),r&&(r.textContent=String(t)),i&&(i.textContent=`x = ${t}, y = g^${t} mod p = ${n}`)}n.addEventListener(`input`,()=>y(Number(n.value))),document.getElementById(`viz-random`)?.addEventListener(`click`,()=>{let e=Number(a(c));n.value=String(e),y(e)}),y(Number(n.value))})(),(()=>{let t=document.getElementById(`wt-steps`),r=document.getElementById(`wt-prev`),i=document.getElementById(`wt-next`),o=document.getElementById(`wt-restart`),s=document.getElementById(`wt-progress`);if(!t||!r||!i||!o||!s)return;let c=t,l=s,u=r,d=i,{p:f,g:p,q:m}=v;function h(){let t=a(m),r=e(p,t,f),i=42n,o=a(m),s=e(p,o,f),c=e(r,o,f),l=i*c%f,u=e(s,t,f),d=n(u,f);return{x:t,y:r,m:i,k:o,c1:s,yk:c,c2:l,s:u,sInv:d,mRec:l*d%f}}let g=[{title:`Public parameters`,narration:`ElGamal lives in a cyclic group. Everyone publicly agrees on a large prime p and a generator g. (We use the toy group so the numbers stay small and readable.)`,math:()=>`p = ${G(f)}\ng = ${G(p)}`},{title:`Key generation`,narration:`Alice picks a secret private key x at random, then publishes her public key y = g^x mod p. Recovering x from y is the discrete-log problem — infeasible at real key sizes.`,math:e=>`private  x = ${G(e.x,`priv`)}\npublic   y = g^x mod p = ${G(e.y,`pub`)}`},{title:`The message`,narration:`Bob wants to send the secret value m to Alice, encoded as a number with 1 ≤ m < p.`,math:e=>`m = ${G(e.m,`m`)}`},{title:`Ephemeral key`,narration:`For THIS message only, Bob draws a fresh random k. Reusing k across messages is catastrophic — the Security Lab below shows exactly how it breaks.`,math:e=>`k = ${G(e.k,`k`)}`},{title:`First ciphertext component`,narration:`Bob computes the first half of the ciphertext from the ephemeral key alone.`,math:e=>`c1 = g^k mod p = ${G(e.c1,`c1`)}`},{title:`Second ciphertext component`,narration:`Bob masks the message with the shared value y^k, which only someone who knows x (or k) can reproduce.`,math:e=>`y^k mod p = ${G(e.yk)}\nc2 = m · y^k mod p = ${G(e.c2,`c2`)}`},{title:`Transmit`,narration:`Bob sends the ciphertext pair. An eavesdropper sees only these two numbers — never m, x, or k.`,math:e=>`ciphertext (c1, c2) = (${G(e.c1,`c1`)}, ${G(e.c2,`c2`)})`},{title:`Reconstruct the mask`,narration:`Alice uses her private key to rebuild the same mask: s = c1^x = g^(kx) = y^k. Only the holder of x can do this.`,math:e=>`s = c1^x mod p = ${G(e.s)}\nsame value as y^k: ${K(e.s===e.yk)}`},{title:`Invert the mask`,narration:`Alice computes the modular inverse of the mask so she can divide it back out.`,math:e=>`s⁻¹ mod p = ${G(e.sInv)}`},{title:`Recover the message`,narration:`Multiplying the inverse mask back out reveals the plaintext — and it matches what Bob sent.`,math:e=>`m = c2 · s⁻¹ mod p = ${G(e.mRec,`m`)}\nrecovered original message: ${K(e.mRec===e.m)}`}],_=h(),y=0;function b(){c.innerHTML=g.slice(0,y+1).map((e,t)=>`
        <div class="wt-step${t===y?` current`:``}">
          <h3>${t+1}. ${W(e.title)}</h3>
          <p class="wt-narr">${W(e.narration)}</p>
          <pre class="output">${e.math(_)}</pre>
        </div>`).join(``),l.textContent=`Step ${y+1} of ${g.length}`,u.disabled=y===0,d.disabled=y===g.length-1}i.addEventListener(`click`,()=>{y<g.length-1&&(y+=1,b())}),r.addEventListener(`click`,()=>{y>0&&(--y,b())}),o.addEventListener(`click`,()=>{_=h(),y=0,b()}),b()})(),me.addEventListener(`change`,()=>{R=xe(),z=null,B=null,H=null,he.textContent=`Not generated`,ge.textContent=`Not generated`,q(k,`${W(R.label)} selected. Generate keys to begin.`),A.textContent=``,A.removeAttribute(`data-state`),q(j,`Encrypt a message first, then re-randomize.`)}),D.addEventListener(`change`,()=>{D.value===`text`?(O.value=`yes`,O.setAttribute(`inputmode`,`text`)):(O.value=`42`,O.setAttribute(`inputmode`,`numeric`))}),U(`keygen-btn`).addEventListener(`click`,()=>{R=xe(),z=l(R),Se(),q(k,`Keypair generated in ${W(R.label)}.`)});function we(t=!1){try{let n=Z(),r=Ce(),i=d(r,n.publicKey,R);H=B,B=i.ciphertext,V=r,ye=i.ephemeralK;let a=e(n.publicKey,i.ephemeralK,R.p),o=H?`Previous c1 = ${G(Y(H.c1),`c1`)}\nNew c1      = ${G(Y(i.ciphertext.c1),`c1`)}\nDifferent: ${K(H.c1!==i.ciphertext.c1)}`:`Run "Encrypt Again" to observe non-determinism directly.`;q(k,[`Message m = ${G(Y(r),`m`)}`,`k = ${G(Y(i.ephemeralK),`k`)}`,`c1 = g^k mod p = ${G(Y(i.ciphertext.c1),`c1`)}`,`c2 = m · y^k mod p = ${G(Y(r*a%R.p),`c2`)}`,`Ciphertext ${X(i.ciphertext)}`,t?`Same plaintext encrypted again.`:``,o].filter(Boolean).join(`
`))}catch(e){J(k,e)}}U(`encrypt-btn`).addEventListener(`click`,()=>we(!1)),U(`encrypt-again-btn`).addEventListener(`click`,()=>we(!0)),U(`decrypt-btn`).addEventListener(`click`,()=>{try{let t=Z();if(!B||V===null||ye===null)throw Error(`No ciphertext available. Encrypt first.`);let r=e(B.c1,t.privateKey,R.p),i=n(r,R.p),a=f(B,t.privateKey,R),o=D.value===`text`?`\nDecoded text: "${W(m(a,R))}"`:``;q(A,[`Ciphertext ${X(B)}`,`s = c1^x mod p = ${G(Y(r))}`,`s⁻¹ mod p = ${G(Y(i))}`,`m = c2 · s⁻¹ mod p = ${G(Y(a),`m`)}`,`Recovered original: ${K(a===V)}`,o].join(`
`))}catch(e){J(A,e)}}),U(`homo-btn`).addEventListener(`click`,()=>{try{let e=BigInt(U(`homo-m1`).value),t=BigInt(U(`homo-m2`).value);if(e<=0n||t<=0n||e>=v.p||t>=v.p)throw Error(`Use values in range 1..2038 for toy homomorphic demo.`);let n=l(v),r=d(e,n.publicKey,v).ciphertext,i=d(t,n.publicKey,v).ciphertext,a=h(r,i),o=f(a,n.privateKey,v),s=e*t%v.p;q(M,[`CT1 = E(${G(e,`m`)}) = ${X(r)}`,`CT2 = E(${G(t,`m`)}) = ${X(i)}`,`CT3 = CT1 ⊗ CT2 = ${X(a)}`,`Decrypt(CT3) = ${G(o,`m`)}`,`Expected m1·m2 mod p = ${G(s,`m`)}`,`Match: ${K(o===s)}`].join(`
`))}catch(e){J(M,e)}}),U(`rerand-btn`).addEventListener(`click`,()=>{try{let e=Z();if(!B||V===null)throw Error(`Encrypt first to create a ciphertext to re-randomize.`);let t=g(B,e.publicKey,R),n=f(B,e.privateKey,R),r=f(t,e.privateKey,R);q(j,[`Original:    ${X(B)}`,`Re-random.:  ${X(t)}`,`Ciphertext changed: ${K(B.c1!==t.c1||B.c2!==t.c2)}`,`Decrypt(original) = ${G(Y(n),`m`)}`,`Decrypt(rerandom) = ${G(Y(r),`m`)}`,`Same plaintext: ${K(n===r)}`].join(`
`)),B=t}catch(e){J(j,e)}}),U(`mixnet-btn`).addEventListener(`click`,()=>{try{let e=l(v),t=[`Alice`,`Bob`,`Charlie`],n=[11n,19n,29n].map((n,r)=>({label:t[r],ct:d(n,e.publicKey,v).ciphertext,m:n})),r=n.map(t=>({...t,ct:g(t.ct,e.publicKey,v)})),i=[r[2],r[0],r[1]];q(N,[`Inputs:`,...n.map(e=>`${W(e.label)}: ${X(e.ct)}`),``,`Outputs after re-randomize + shuffle:`,...i.map((t,n)=>`Out${n+1}: ${X(t.ct)} → decrypts to ${G(f(t.ct,e.privateKey,v),`m`)}`),``,`Observer sees changed bytes and changed order, but cannot link sender to output without secret permutation info.`].join(`
`))}catch(e){J(N,e)}});function Q(e){let t=U(e).value.trim();if(!/^\d+$/.test(t))throw Error(`Enter a positive integer.`);let n=BigInt(t);if(n<=0n||n>=v.p)throw Error(`Use values in range 1..2038 for the toy group.`);return n}U(`crack-btn`).addEventListener(`click`,()=>{try{let t=l(v),{x:n,steps:r}=ne(v,t.publicKey),i=n!==null&&e(v.g,n,v.p)===t.publicKey;q(P,[`Public key y = ${G(t.publicKey,`pub`)}   (this is all the attacker sees)`,`Goal: find x such that g^x ≡ y (mod ${v.p}),  g = ${v.g}`,``,`Recovered x = ${G(n??`not found`,`priv`)}`,`Actual x    = ${G(t.privateKey,`priv`)}`,`Key recovered: ${K(i&&n===t.privateKey)}`,`Baby-step/giant-step operations: ${G(r)}  (naive search would scan up to ${v.q})`,``,`The toy subgroup order is ~1019, so this finishes instantly.`,`RFC 3526 Group 14 has order ≈ 2^2047 — the same attack would need more`,`operations than there are atoms in the observable universe. That gap is the security.`].join(`
`))}catch(e){J(P,e)}}),U(`kreuse-btn`).addEventListener(`click`,()=>{try{let e=Q(`kreuse-m1`),t=Q(`kreuse-m2`),r=l(v),i=a(v.q),o=u(e,r.publicKey,v,i),s=u(t,r.publicKey,v,i),c=e*s.c2%v.p*n(o.c2,v.p)%v.p;q(F,[`CT1 = E(${G(e,`m`)}) = ${X(o)}`,`CT2 = E(${G(t,`m`)}) = ${X(s)}`,`Shared c1 (the tell): ${K(o.c1===s.c1)} → ${G(o.c1,`c1`)} = ${G(s.c1,`c1`)}`,``,`Attacker knows only m1 and both ciphertexts, then computes`,`m2 = m1 · c2₂ · c2₁⁻¹ mod p:`,`Recovered m2 = ${G(c,`m`)}`,`Actual m2    = ${G(t,`m`)}`,`Secret leaked: ${K(c===t)}  — no private key was used.`].join(`
`))}catch(e){J(F,e)}}),U(`maul-btn`).addEventListener(`click`,()=>{try{let e=Q(`maul-secret`),t=Q(`maul-factor`),n=l(v),r=d(e,n.publicKey,v).ciphertext,i={c1:r.c1,c2:r.c2*t%v.p,group:v},a=f(i,n.privateKey,v),o=e*t%v.p;q(I,[`Honest ciphertext  E(${G(e,`m`)}) = ${X(r)}`,`Attacker multiplies c2 by t = ${G(t)} (private key never touched):`,`Forged ciphertext  ${X(i)}`,``,`Owner decrypts forged CT → ${G(a,`m`)}`,`Predicted m·t mod p     → ${G(o,`m`)}`,`Decryption silently altered: ${K(a===o&&t!==1n)}`,``,`The owner has no way to detect tampering — plain ElGamal provides no integrity.`,`(The authenticated-ElGamal exhibit below fixes exactly this.)`].join(`
`))}catch(e){J(I,e)}}),U(`auth-btn`).addEventListener(`click`,async()=>{try{let e=Q(`auth-secret`),t=Q(`auth-factor`),n=l(v),r=await se(e,n.publicKey,v),i=await S(r,n.privateKey,v),a={...r,c2:r.c2*t%v.p},o=await S(a,n.privateKey,v);q(L,[`Authenticated ciphertext ${X(r)}`,`MAC tag = ${G(r.tag.slice(0,32)+`…`)}  (keyed by the shared secret)`,``,`Honest decrypt → authentic: ${K(i.authentic)}, m = ${G(i.message??0n,`m`)}`,``,`Attacker multiplies c2 by t = ${G(t)} (the same malleability attack):`,`Forged ciphertext ${X(a)}`,`Owner verifies MAC → authentic: ${K(o.authentic)}`,o.authentic?`t = 1 leaves the ciphertext unchanged, so it still verifies. Try t ≥ 2.`:`Tampering REJECTED — the recomputed MAC does not match, so decryption is refused.`,``,`Integrity restored. The tradeoff: this ciphertext can no longer be re-randomized`,`or homomorphically multiplied without breaking the tag.`].join(`
`))}catch(e){J(L,e)}});function $(e){let t=U(e).value.trim();if(!/^\d+$/.test(t))throw Error(`Enter a positive integer.`);let n=BigInt(t);if(n<=0n||n>=C.n)throw Error(`Use values in range 1..2037 for the signature demo.`);return n}U(`sign-btn`).addEventListener(`click`,()=>{try{let e=$(`sign-msg`),t=le(),{sig:n,k:r}=ue(e,t.x),i=de(e,n,t.y),a=e%(C.n-1n)+1n,o=de(a,n,t.y);q(_e,[`Public key y = g^x mod p = ${G(t.y,`pub`)}   (private x = ${G(t.x,`priv`)})`,``,`Sign h = ${G(e,`m`)}  (ephemeral k = ${G(r,`k`)}, kept secret)`,`Signature (r, s) = (${G(n.r,`c1`)}, ${G(n.s,`c2`)})`,``,`Verify g^h ≡ y^r · r^s (mod p): ${K(i)}`,`Same signature on a different message h = ${G(a,`m`)}: ${K(o)}`,``,`A valid signature binds the message to the private key; changing the message breaks it.`].join(`
`))}catch(e){J(_e,e)}}),U(`sigreuse-btn`).addEventListener(`click`,()=>{try{let e=$(`sigreuse-m1`),t=$(`sigreuse-m2`);if(e===t)throw Error(`Use two different messages so the signatures differ.`);let n=le(),r=null,i=null,o=null;for(let s=0;s<2e3&&i===null;s+=1){let s=a(C.n);try{i=E(e,n.x,s),o=E(t,n.x,s),r=s}catch{i=null}}if(!i||!o||r===null)throw Error(`Could not sample a usable nonce; try again.`);let{k:s,x:c}=fe(e,i.s,t,o.s,i.r,n.y);q(ve,[`Signature 1 on h1 = ${G(e,`m`)}: (r, s) = (${G(i.r,`c1`)}, ${G(i.s,`c2`)})`,`Signature 2 on h2 = ${G(t,`m`)}: (r, s) = (${G(o.r,`c1`)}, ${G(o.s,`c2`)})`,`Shared r (the tell): ${K(i.r===o.r)}`,``,`From s1 - s2 ≡ (h1 - h2)·k⁻¹ the attacker solves for k, then for x:`,`Recovered k = ${G(s??`none`,`k`)}   (actual ${G(r,`k`)})`,`Recovered private key x = ${G(c??`none`,`priv`)}`,`Actual private key x    = ${G(n.x,`priv`)}`,`Full key compromised: ${K(c===n.x)}  — every future signature is now forgeable.`].join(`
`))}catch(e){J(ve,e)}});